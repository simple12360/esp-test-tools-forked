...
The Bluetooth LE Adaptivity Test is used to ensure that the device operates in a frequency hopping manner and that the Power Spectral Density (PSD) of the Bluetooth LE signal is greater than 10 dBm/MHz, meeting certain parameter requirements to avoid interference with other wireless devices. This test is mainly used in CE and SRRC certification tests.

.. note::

  - When the PSD of the Bluetooth LE signal is less than 10 dBm/MHz, you can choose to base it on the equivalent occupancy rate, and there is no need to perform the Bluetooth LE Adaptivity Test.
  - When the PSD of the Bluetooth LE signal is greater than 10 dBm/MHz, you can choose to use the Listen Before Talk (LBT) mechanism based on frequency hopping for the Bluetooth LE Adaptivity Test.
...
- In the test, use the {IDF_TARGET_NAME} module as the test device (Slave) to establish a connection with the device under test (Master). The Slave and Master burn the same firmware and use serial port commands to distinguish.

- The Test System refers to the adaptivity test system. After the Master and Slave successfully connect through serial port commands, the test can begin.
...
1. Open the :ref:`download-tool` tool.

2. Set the ChipType, Com Port, Baud Rate, click Open, and choose to download to Flash.

3. Burn the {IDF_TARGET_BLE_ADAPTIVITY_FIRMWARE} to 0x0 via UART.
...
The Bluetooth LE Adaptivity Test needs to be tested after establishing a connection by entering the corresponding serial port commands in the Mater and Slave devices.

Enter the corresponding commands in the Slave and Master devices in turn:

1. **Slave device**

::

  \\Start the broadcast of the pairing test device
...
::
  ...
  \\Establish connection, configure the rate to 1 Mbps (if you need to configure to 2 Mbps, modify the parameter to -x 2 -y 2), set the power level (-v 13)
  bleconn -T -z start -x 1 -y 1 -n 1 -i 0x6-0x6 -v 13

  \\Configure power, default setting is level 13 (-e parameter should be consistent with the -v parameter in the previous command)
